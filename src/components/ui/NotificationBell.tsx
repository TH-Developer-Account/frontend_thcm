...
function NotificationItem({
  ...
  const isDownloadable = notification.metadata?.downloadable === true;

  return (
    <div
      className={`w-full flex gap-3 px-4 py-3 border-b border-gray-100 last:border-b-0 transition-colors hover:bg-gray-50 ${
      ...
      >
        ...
        {isDownloadable && (
          <button
            ...
            className="mt-2 text-xs font-medium text-orange-600 border border-orange-200 rounded-md px-2.5 py-1 hover:bg-orange-50"
          >
            Download
          </button>
        )}
...
export default function NotificationBell() {
  ...
  const handleDownload = (notification: Notification) => {
    ...
    downloadExportFile(notification.metadata?.logId as string);
  };
