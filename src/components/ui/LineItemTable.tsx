...
export default function LineItemTable({
  ...
  const handleParticularChange = (
    ...
    let selectedOption: LineItemOption | undefined = opt;

    if (fieldName === "partNumber") {
      selectedOption = particularOptions.find(
        (i) => i.partNumber === opt.label,
      );
    }
  ...
  const partNumbers = particularOptions.map((item) => ({
    value: item.particular, // what gets submitted
    label: item.partNumber, // what is shown in dropdown
  }));
