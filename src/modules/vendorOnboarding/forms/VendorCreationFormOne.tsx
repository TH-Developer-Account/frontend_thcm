...
import BankAsyncSelect, {
  type BankOption,
} from "../../../components/forms/BankAsyncSelect";
import BranchAsyncSelect, {
  type BranchOption,
} from "../../../components/forms/BranchAsyncSelect";
...
const VendorCreationFormOne = ({
  ...
  const [selectedBank, setSelectedBank] = React.useState<BankOption | null>(
    values.bank ? { value: values.bank, label: values.bank } : null,
  );
  const [selectedBranch, setSelectedBranch] =
    React.useState<BranchOption | null>(
      values.branch
        ? {
            value: values.ifscCode ?? values.branch,
            label: values.branch,
  ...
  const handleBankChange = (option: BankOption | null) => {
    ...
    setSelectedBranch(null); // changing bank invalidates any previously chosen branch
    onChange?.("bank", option?.value ?? "");
    onChange?.("branch", "");
    onChange?.("ifscCode", "");
    onChange?.("bankAddress", "");
  };

  const handleBranchChange = (option: BranchOption | null) => {
    setSelectedBranch(option);
    onChange?.("branch", option?.label ?? "");
    onChange?.("ifscCode", option?.ifsc ?? "");
    onChange?.("bankAddress", option?.address ?? "");
  };
      ...
      >
        ...
        <div className="vendor-onboarding-form-grid">
          <BankAsyncSelect
            ...
            label="Bank"
            required
            helperText="Search and select the vendor's bank."
            value={selectedBank}
          ...
          <BranchAsyncSelect
            ...
            label="Branch"
            required
            helperText="Search and select the branch."
            bankName={selectedBank?.value ?? null}
