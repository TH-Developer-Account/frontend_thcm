...
export function useVendorCreationSummaryController({
	...
	}, [approveStageMutation.mutateAsync, currentStageId, onApprove, showToast]);
	...
	const handleReasonConfirm = React.useCallback(
		...
		[
			clarifyStageMutation.mutateAsync,
			closeReasonModal,
			currentStageId,
			onClarify,
			showToast,
...
export function useVendorCreationForm({
	...
	const isThcmEmployee = !isPublicForm && role === "THCM_EMPLOYEE";
	...
	*/
	...
	const submitForApproval = React.useCallback(async () => {
		...
		const selectedWorkflowId =
			pendingWorkflowSelection?.attachInput.workflowId?.trim() ?? "";
		...
		try {
			// Steps 1 and 2 have already persisted the vendor data. Initial
			// assignment also starts Stage 1, so it must not be activated again.
			if (
				!isClarifiedResubmission &&
				!hasAssignedWorkflow &&
				selectedWorkflowId
