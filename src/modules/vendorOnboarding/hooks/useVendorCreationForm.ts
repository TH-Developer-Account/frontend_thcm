...
import {
	...
	getErrorMessage,
	getMissingDocuments,
	normalizeAccountNumber, // ADDED
	toYesNo,
	validateConfirmAccountNumber,
	validateFormOneField, // ADDED
...
export function useVendorCreationForm({
	...
	*/
	...
	const validateFormOneBeforeSubmit = React.useCallback((): boolean => {
		const errors = validateFormOneForSubmit(
			formOneValues,
	...
	const saveThcmDetails = async () => {
		...
		// Moving between steps must not persist the form. The complete vendor
		// payload is updated exactly once from submitForApproval, immediately
		// before send-for-approval. Explicit "Save draft" remains the only
		// earlier persistence path.
		next();
	};
	...
	|--------------------------------------------------------------------------
	| 1. Assign a workflow (fresh form) or activate it (clarification).
	| 2. Update the complete vendor payload exactly once.
	| 3. Only after both succeed, send the record for approval.
	|
	| Await every operation. A failure stops the chain, so send-for-approval can
	| never run without a successfully prepared workflow.
	|--------------------------------------------------------------------------
	*/
	const submitForApproval = React.useCallback(async () => {
		...
		try {
			...
			// Persist exactly once and only at the final boundary, after the workflow
			// action succeeded and immediately before status advancement.
			...
			// Status advances only after workflow preparation and update both succeed.
