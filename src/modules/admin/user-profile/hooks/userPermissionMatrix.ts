...
const initializePermState = (
  ...
  // permState is a MODULE-only grid — an app-scope ("admin of this whole
  // app") row has scope: "APP" and no checkbox cell to land in, so it's
  // filtered out here rather than fed into buildPermStateFromApi. This also
  // narrows the type so moduleKey is guaranteed present for what remains.
  const moduleScopedPermissions = existingPermissions.filter(
    (p): p is WorkspacePermission & { moduleKey: string } =>
      p.scope === "MODULE",
  );

  const fromApi = buildPermStateFromApi(moduleScopedPermissions);
