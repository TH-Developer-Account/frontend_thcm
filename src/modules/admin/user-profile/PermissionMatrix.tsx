...
interface Props {
	...
	colState: (action: Action) => { all: boolean; some: boolean };
	...
	toggleColumnAll: (action: Action) => void;
	...
	onSavePermissions: (data: any) => void;
}
...
export default function PermissionMatrix({
	...
	const handleReset = () => {
		...
		onSavePermissions(emptyState); // optional if parent tracks state
	...
	return (
		<div className="bg-gradient-to-b from-white to-slate-50 rounded-2xl border border-slate-200 overflow-hidden shadow-md">
									...
									return (
										...
										>
											<div className="flex items-center gap-3 pl-14 pr-5 py-2.5">
												...
												<Checkbox
													checked={ms.all}
													indeterminate={ms.some}
													onChange={() => toggleModule(app.key, mod.key)}
												/>
			...
			<div className="flex justify-end gap-3 px-6 py-4 border-t border-slate-200 bg-white">
				<button
					onClick={handleReset}
					className="px-5 py-2.5 rounded-lg border border-slate-300 text-slate-600 text-sm font-semibold hover:bg-slate-50 transition"
				>
					Reset
				</button>
