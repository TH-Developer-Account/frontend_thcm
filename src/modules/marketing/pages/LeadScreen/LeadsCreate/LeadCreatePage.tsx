...
const LEAD_INFO_STORAGE_KEY = "LeadInfo";
...
export default function LeadCreatePage() {
		...
		try {
			...
			const payload = {
				...
				leads: items.map((item) => ({
					name: item.leadName.trim(),
					phone: item.leadPhoneNumber.trim(),
					email: item.leadEmail.trim(),
					notes: item.notes,
				})),
			};
			...
			if (isEditMode && leadInfo?.leadId) {
				...
				await ServerAxios.post(`/leads/create-leads`, payload);
			}
	...
	if (!leadInfo?.epcId) {
		console.log("EPC lead info", leadInfo);
		return (
